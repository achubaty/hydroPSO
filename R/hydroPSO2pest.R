...
.read_observations <- function(fname = "Observations.txt") {
  ...
  if (NCOL(x) == 1) {
    ...
    x <- coredata(read.zoo(fname, header = FALSE))
  } # zoo::coredata ;  zoo::read.zoo
...
################################################################################
hydroPSO2pest <- function(param.files = "ParamFiles.txt",
  ...
  for (t in 1:ntpls) {
    ...
    .modify.tpl(
      tpl.fname = tpl.fname, modelin.fname = fname.in, paramfiles = x,
