...
#           27-Jan-2022                                                        #
################################################################################
verification <- function(fn = "hydromod",
                         par,
                         ...,
                         control = list(),
                         model.FUN = NULL,
  ...
  if (is.matrix(par) | is.data.frame(par)) {
    ...
    if (is.null(names(par))) {
      ...
      Parameter.names <- names(par)
    }
  } else {
    stop("Invalid argument: 'class(par)' must be in c('numeric', 'matrix', 'data.frame')")
  }
  ...
  if (fn.name == "hydromodInR") { # Evaluating an R-based model
    ...
    if (parallel == "none") {
      out <- pbapply::pblapply(
        X = 1:nparamsets, FUN = hydromodInR.eval,
        Particles = par,
        model.FUN = model.FUN,
  ...
  if (fn.name == "hydromod") {
    ...
    if (parallel == "none") {
      ...
      out <- pbapply::pblapply(
        X = 1:nparamsets, FUN = hydromod.eval,
        Particles = par,
        iter = 1,
        npart = nparamsets,
        maxit = 1,
  ...
  for (p in 1:nparamsets) {
    ...
    if ((fn.name != "hydromod") & (fn.name != "hydromodInR")) {
      ...
      suppressWarnings(writeLines(as.character(c(p, tmp, formatC(sims[[p]], format = "E", digits = digits, flag = " "))), OFout.Text.file, sep = "  "))
    }
    writeLines("", OFout.Text.file) # writing a blank line with a carriage return
