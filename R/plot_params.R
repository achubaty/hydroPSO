...
plot_params.default <- function(params,
  ...
  if (ptype != "pairs") {
    for (i in 1:nparams) {
      if (ptype == "dottyplot") {
        plot(params[, i], gofs,
          type = "p", col = col, cex = cex, cex.main = cex.main,
        ...
        if (!is.na(beh.thr)) {
          ...
          axis(
            side = 4, at = beh.thr, labels = round(beh.thr, 2), col = beh.col,
        ...
        boxplot(params[, i],
          col = "lightblue", cex = cex, cex.main = cex.main,
          cex.axis = cex.axis, cex.lab = cex.lab, font.lab = 2,
          xlab = colnames(params)[i], ylab = ylab
        )
        ...
        if (length(unique(params[, i])) != 1) {
          ...
          message(
            "Vioplot is not possible: all elements of '",
