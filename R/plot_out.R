...
plot_out <- function(sim, obs,
  ...
  if (do.png) {
    if (ptype == "corr") {
      png(filename = png.fname, width = min(png.width, png.height), height = min(png.width, png.height), res = png.res)
  ...
  if (ptype == "corr") {
    ...
    plot(obs, sim,
      xlab = "Observations", ylab = "Best Simulation",
    ...
    # Plotting Sim vs Obs
    hydroGOF::ggof(
      sim = sim, obs = obs, main = main, ftype = ftype, FUN = FUN,
      cex.main = cex.main, cex.axis = cex.axis, cex.lab = cex.lab, leg.cex = leg.cex
    ) # dates.fmt="%Y-%m-%d")
