...
params2ecdf.default <- function(params,
  ...
  for (f in 1:nfigs) {
    ...
    if (nparam.bak <= 21) {
      ...
      ifelse(f * 21 <= nparam.bak, L <- f * 21, L <- nparam.bak)
    }
    ...
    for (i in 1:nparam) {
      ...
      if (verbose) {
        message(
          "[ Computing the ", char1, " for '",
          format(param.names[p], width = ncharmax),
          "' , ", p, "/", nparam.bak, " => ",
          format(round(100 * i / nparam.bak, 2),
            width = 5,
      ...
      if (plot) {
        ...
        plot(p.ecdf$x, p.ecdf$ecdf,
          xlab = param.names[p], main = main.loc,
        ...
        Axis(
          side = 2, at = seq(0.0, 1, by = 0.05), labels = FALSE,
          cex.axis = cex.axis, cex.lab = cex.lab
        )
        Axis(
          side = 2, at = seq(0.0, 1, by = 0.1), labels = seq(0.0, 1, by = 0.1),
        ...
        if (!is.null(obs) & is.numeric(obs)) {
          ...
          leg.txt <- c(
            paste("Obs    : ", round(quantile.obs, 3)),
            paste("Q50 sim: ", round(quantile.sim, 3)),
            paste("Bias   : ", round(bias, 1), "[%]", sep = " ")
          )
          legend(leg.pos,
            legend = leg.txt, inset = 0.02, bty = "n",
