...
hydromod.eval.SA <- function(j, Thetas, nparamsets,
  ...
  while (!gof.is.numeric) {
    ...
    gof.is.numeric <- ifelse(is.finite(out[[1]]), TRUE, FALSE)
...
lhoat <- function(fn = "hydromod",
  ...
  if (parallel != "none") {
    ...
    parallel.pkg <- ifelse(parallel == "parallelWin", "parallel", parallel)
    ...
    if ((parallel == "parallel") | (parallel == "parallelWin")) {
      logfile.fname <- file.path(drty.out, "parallel_logfile.txt")
    ...
    if (parallel == "parallel") {
      if (is.null(cl)) {
        cl <- ifelse(write2disk,
                     makeForkCluster(nnodes = par.nnodes, outfile = logfile.fname),
                     makeForkCluster(nnodes = par.nnodes)
        )
      }
    } else if (parallel == "parallelWin") {
      if (is.null(cl)) {
        cl <- ifelse(write2disk,
                     makeCluster(par.nnodes, outfile = logfile.fname),
                     makeCluster(par.nnodes)
        )
      }
  ...
  if (write2disk) {
    # File 'LH_OAT-out.txt' #
    model.out.text.fname <- file.path(drty.out, "LH_OAT-out.txt")
    ...
    gof.text.fname <- file.path(drty.out, "LH_OAT-gof.txt")
  ...
  if (fn.name != "hydromod") {
    ...
    if (parallel == "none") {
      ...
      out <- clusterApply(
        cl = cl, x = 1:nparamsets, fun = hydromod.eval.SA,
        Thetas = Xn,
        nparamsets = nparamsets,
        N = N, X.Boundaries = X.Boundaries,
        write2disk = write2disk,
      ...
      if (write2disk) {
        ...
        suppressWarnings(
          writeLines(as.character(c(
            formatC(out[[j]][["GoF"]], format = "E", digits = digits, flag = " "), # GoF
    ...
    if ((fn.name != "hydromod")) {
      ...
      for (j in 1:nparamsets) {
        ...
        if (is.finite(temp.gof)) {
          suppressWarnings(
            writeLines(as.character(c(
              formatC(temp.gof, format = "E", digits = digits, flag = " "),
  ...
  Ranking <- data.frame(
    RankingNmbr = format(as.character(1:P), width = 11, justify = "left"),
