...
.pst2paramfiles <- function(drty.model, tpls, inputs, param.names,
  ...
  field.names <- c(
    "ParameterNmbr", "ParameterName", "Filename",
  ...
  for (p in 1:nparam) {
    message(
      "[ Processing parameter ", format(p, width = nparam.width, justify = "left"), "/",
      format(nparam, width = nparam.width, justify = "left"), ": ", param.names[p], " ... ]"
    )

    for (f in 1:ntpl) {
      message(
        "   [ Processing   file    ", format(f, width = ntpl.width, justify = "left"), "/",
...
pest2hydroPSO <- function(pst.fname,
  ...
  L <- length(ini.index)
  if (L > 0) {
    suppressWarnings(tmp <- read.table(
      file = pst.fname, skip = ini.index, header = FALSE, nrows = nparam, colClasses
    ...
    stop("Invalid pst file: ", params.stg, " does not exist !")
  }
  ...
  .pst2paramranges(
    drty.model = drty.model, names = param.names, ini = param.ini,
  ...
  if (L > 0) {
    suppressWarnings(tmp <- read.table(
      file = pst.fname, skip = ini.index, header = FALSE, nrows = nobs, colClasses
      = c("NULL", "numeric", "NULL", "NULL"), fill = TRUE, na.strings = ""
    ))

    obs <- tmp[, 1]
  } else {
    stop("Invalid pst file: ", obs.stg, " does not exist !")
  }
  ...
  if (L > 0) {
    ...
    stop("Invalid pst file: ", model.stg, " does not exist !")
  }
  ...
  if (L > 0) {
    suppressWarnings(tmp <- read.table(
      file = pst.fname, skip = ini.index, header = FALSE, nrows = ntpl + nins, colClasses
    ...
    stop("Invalid pst file: ", io.stg, " does not exist !")
  }

  # Writing ParamFiles.txt
  .pst2paramfiles(
    drty.model = drty.model, tpls = tpls, inputs = inputs,
