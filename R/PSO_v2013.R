...
computeCurrentXmaxMin <- function(x.best.part) {
  ...
  x.min <- sapply(1:n, function(i, y) {
    min(y[, i], na.rm = TRUE)
  }, y = x.best.part)
  x.max <- sapply(1:n, function(i, y) {
    max(y[, i], na.rm = TRUE)
  }, y = x.best.part)
...
hydromod.eval <- function(part, Particles, iter, npart, maxit,
  ...
  if (iter / REPORT == floor(iter / REPORT)) {
    ...
    if (verbose) {
      message(
        "[Iter: ", format(iter, width = 4, justify = "left"), "/", maxit,
...
hydroPSO <- function(par,
  ...
  if (is.character(fn) | is.function(fn)) {
    if (is.character(fn)) {
      if (fn == "hydromod") {
        ...
        stop("Invalid argument: valid character values for 'fn' are only: c('hydromod', 'hydromodInR')")
      }
  ...
  if (use.IW) {
    ...
    if (length(w) == 2) {
      ...
      stop("Invalid argument: 'length(w)' must be 1 or 2 !!")
    }
  ...
  V <- InitializateV(
    npart = npart, x.MinMax = X.Boundaries, v.ini.type = Vini.type,
    Xini = X
  )
  ...
  while ((iter <= maxit) && (!abstol.conv) && (!reltol.conv) && (nfn.eff <= maxfn)) {
    ...
    if (!use.IW) {
      ...
      if ((IW.type == "linear") | (IW.type == "non-linear")) {
        w <- compute.value.with.iter(
          iter = iter.tv, niter = niter.tv,
    ...
    # lambda
    if (use.TVlambda) {
      lambda <- compute.value.with.iter(
        iter = iter.tv, niter = niter.tv,
    ...
    if ((fn.name != "hydromod") & (fn.name != "hydromodInR")) {
      ...
      if (parallel == "none") {
        ...
        out <- parallel::clusterApply(
          cl = cl, x = 1:npart, fun = hydromod.eval,
          Particles = Xn,
          iter = iter,
          npart = npart,
          maxit = maxit,
      ...
      if (parallel == "none") {
        ...
        out <- parallel::clusterApply(
          cl = cl, x = 1:npart, fun = hydromodInR.eval,
          Particles = Xn,
          model.FUN = model.FUN,
          model.FUN.args = model.FUN.args
        ) # sapply END
    ...
    for (j in index.part.upd) {
      if (write2disk) {
        ...
        if (is.finite(GoF)) {
          ...
          writeLines(as.character(c(iter, j, "NA", "NA")), OFout.Text.file, sep = "  ")
        }
        writeLines("", OFout.Text.file)
        ...
        if (is.finite(GoF)) {
          ...
          suppressWarnings(writeLines(as.character(c(
            iter, j, "NA",
            formatC(Xn[j, ], format = "E", digits = digits, flag = " ")
          )), Particles.TextFile, sep = "  "))
        }
    ...
    if (plot) {
      ...
      if (iter == 1) {
        plot(X[, 1], X[, 2],
          xlim = X.Boundaries[1, ], ylim = X.Boundaries[2, ],
          main = paste("Iter= ", iter, ". GoF= ",
            format(lgof, scientific = TRUE, digits = digits),
            sep = ""
          ),
    ...
    if ((verbose) & (iter / REPORT == floor(iter / REPORT))) {
      suppressWarnings(
        message(
          "iter:", format(iter, width = nchar(maxit), justify = "right"),
  ...
  names(out)[1:nelements] <- c(
    "par", # "Best.Parameter.Values",
