...
#          04-Jan-2021                                                         #
################################################################################
hydromod <- function(param.values, # numeric vector with the paramter values that will be used in the input files of the hydrological model
  ...
  ParameterValues2InputFiles(
    NewValues = param.values, ParamFiles.fname = param.files,
    verbose = verbose
  )
  ...
  if (do.png) {
    ...
    if (length(find.package("hydroGOF", quiet = TRUE)) != 0) {
      ...
      plot_out(sim = sim, obs = obs, ptype = "corr")
    }
